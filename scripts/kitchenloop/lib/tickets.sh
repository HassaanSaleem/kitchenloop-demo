...
ticket_create() {
  ...
  case "$provider" in
    ...
    local|none)
      ...
      local f
      f=$(_local_backlog_file)
      # $RANDOM is a per-shell PRNG: subshells forked from one parent in the
      # same second inherit the same seed state and draw the SAME value,
      # colliding on id. /dev/urandom is a kernel source (no inherited seed),
      # plus a collision re-roll loop against the current backlog as a
      # defense-in-depth backstop.
      local id attempts=0
      while :; do
        id=$(printf '%s%s%05d' "$(date +%s)" "$$" "$(od -An -N2 -tu2 /dev/urandom | tr -d ' ')")
        local exists
        exists=$(jq --arg i "$id" '[.[] | select(.id == $i)] | length' "$f")
        [ "$exists" -eq 0 ] && break
        attempts=$((attempts + 1))
        if [ "$attempts" -ge 10 ]; then
          echo "ERROR: ticket_create could not generate a unique id after 10 attempts" >&2
          return 1
        fi
      done
      local entry
...
ticket_extract_ids_from_pr() {
  ...
  case "$provider" in
    github)
      ...
      ;;
    local|none)
      # Look for "Ticket: ID" pattern in PR body. IDs are non-numeric for the
      # local provider (e.g. tkt-0001), so match the full slug charset — a
      # [0-9]+ pattern would never match the seeded tickets.
      local body
      body=$(gh pr view "$pr_number" --json body -q '.body' 2>/dev/null || echo "")
      echo "$body" | grep -oE 'Ticket: [A-Za-z0-9_-]+' | sed 's/Ticket: //' | sort -u
      ;;
  esac
}
